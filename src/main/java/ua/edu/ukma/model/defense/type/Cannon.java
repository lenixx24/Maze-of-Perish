...
public class Cannon extends AttackTower {
    ...
    private final double shootDelay = 1.2;

    private final List<CannonBullet> bullets = new ArrayList<>();

    public Cannon(int row, int col) {
        super(row, col, DefenseType.CANNON_TOWER, 10, 10, 10, 10);
    }
    ...
    public void updateTurret(List<Enemy> activeEnemies, GameMap gameMap, int tileSize, double deltaTime) {
        ...
        Iterator<CannonBullet> bIterator = bullets.iterator();
        while (bIterator.hasNext()) {
            CannonBullet bullet = bIterator.next();
    ...
    private void shoot(double startX, double startY, double targetX, double targetY, int tileSize) {
        CannonBullet newBullet = new CannonBullet(startX, startY, targetX, targetY, tileSize);
    ...
    public List<CannonBullet> getBullets() {
        return bullets;
    }

    public static class CannonBullet {
        private double x, y;
        ...
        public CannonBullet(double startX, double startY, double targetX, double targetY, int tileSize) {
            this.x = startX;
            this.y = startY;
            Image bulletImage = new Image(Objects.requireNonNull(getClass().getResourceAsStream("/defense/cannon_bullet.png")));
            this.bulletView = new ImageView(bulletImage);
            this.bulletView.setFitWidth(tileSize * 0.8);
            this.bulletView.setFitHeight(tileSize * 0.8);
        ...
        public boolean collidesWith(Enemy enemy, int tileSize) {
            ...
            return Math.hypot(this.x - enemyCenterX, this.y - enemyCenterY) < (tileSize * 0.5);
        }
